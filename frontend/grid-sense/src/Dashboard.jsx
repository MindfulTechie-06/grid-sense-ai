...
export default function Dashboard() {
  ...
  const [waveData, setWaveData] = useState([]); // Visual oscilloscope data
  ...
  // Engine 1: Infinite Stochastic Engine (replaces dataset)
  useEffect(() => {
    const generatePoint = (prevPt) => {
        const rand = Math.random();
        let state = prevPt ? prevPt.state : 'normal';
        
        // Sticky transition logic: 10% chance to roll a state change per tick
        // Prevents hyper-flashing by allowing states to hold for a few seconds
        if (Math.random() < 0.12) {
        ...
        // Assign extreme high-entropy behaviors
        if (state === 'normal') {
            voltage = 220 + Math.random() * 15;
            current = 4.0 + Math.random() * 2.0;
            frequency = 49.8 + Math.random() * 0.4;
        } else if (state === 'fault') {
            voltage = 180 + Math.random() * 30; // heavy sagging
            current = 6.0 + Math.random() * 3.0; // spikes
            frequency = 48.0 + Math.random() * 1.0; // drops
        } else {
            voltage = 100 + Math.random() * 200; // volatile jumping
            current = 2.0 + Math.random() * 8.0; // wildly uncertain
            frequency = 47.0 + Math.random() * 5.0; // massive broad spectrum jitter
        }
    ...
    }, 600); // 600ms stochastic streaming tick
    return () => clearInterval(interval);
  }, []);

  // Alert System: Trigger exclusively on stochastic state changes
  useEffect(() => {
    ...
    // Fire alert ONLY when crossing an anomaly boundary generated by math
  ...
  // Engine 2: High FPS Oscilloscope Visualizer
  useEffect(() => {
    const interval = setInterval(() => {
       const latest = streamData.length > 0 ? streamData[streamData.length - 1] : { voltage: 0, frequency: 50, current: 0, state: "normal" };

       // Smoothly lerp physical bounds to simulate analogue shifting using exact stochastic values
       ...
       setWaveData(prevWave => {
          ...
          for(let i=0; i<4; i++) {
              ...
              // Force visual anomaly differences based straight on the stochastic state
    ...
    }, 45); // ~22 fps continuous 
    return () => clearInterval(interval);
  }, [streamData]);

  // Execute Neural Inference against FastAPI backend continuously
  useEffect(() => {
    if (streamData.length < 2) return; 

    // Extract windowed array payloads for python
    ...
    const runAnalysis = async () => {
      try {
        ...
        // Safe ignorance to preserve UI flow if backend disconnects
      }
    };
    
    // Hit backend periodically
    if (streamData.length % 2 === 0) runAnalysis();
  }, [streamData]);

  // Derived State Definitions (Hard-coupled to Math)
  ...
  // Manually enforce metric limits based on strict dataset states to guarantee visual changes
  const pqMetrics = {
    rmsVoltage: currentPt.voltage, // Directly from stochastic block
    frequency: currentPt.frequency, // Directly from stochastic block
  ...
  // Strictly enforce UI displays by reading the state block directly
